...
class CityStream(GeoIPStream):
    name = "geoip_city"
    primary_keys = ["network", "date"]
    replication_key = None
    
    @property
    def edition(self) -> dict:
        return self.config['city_edition']
...
class DomainStream(GeoIPStream):
    name = "geoip_domain"
    primary_keys = ["network", "date"]
    replication_key = None
    ...
    @property
    def edition(self) -> dict:
        return self.config['domain_edition']
    ...
    def get_records(self, context: Optional[dict]) -> Iterable[dict]:
        ...
        if r.ok:
            ...
            for f in files:
                ...
                datefilename = folder[-8:]
                date = f"{datefilename[:4]}-{datefilename[4:6]}-{datefilename[6:8]}"
                if f.endswith("GeoIP2-Domain-Blocks-IPv4.csv"):
                    with z.open(f, 'r') as infile:
                        reader = csv.DictReader(TextIOWrapper(infile, 'utf-8'))
                        ...
                        for row in reader:
                            ...
                            row['date'] = date
                            yield self.post_process(row)
                if f.endswith("GeoIP2-Domain-Blocks-IPv6.csv"):
                    with z.open(f, 'r') as infile:
                        reader = csv.DictReader(TextIOWrapper(infile, 'utf-8'))
